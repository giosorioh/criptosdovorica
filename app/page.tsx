...
const CRYPTO_CONFIG = [
  ...
  },
  {
    ...
    coingeckoId: "bitcoin",
    basePrice: 580000, // Preço base para simulação
  },
]
...
// Função para buscar dados reais da API CoinGecko com fallback
const fetchRealCryptoData = async (): Promise<any> => {
  try {
    ...
    const response = await fetch(
      "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,wemix-token&vs_currencies=brl&include_24hr_change=true",
      {
    ...
    if (!data.bitcoin || !data["wemix-token"]) {
    ...
    result["bitcoin"] = {
      ...
      brl_24h_change: data.bitcoin.brl_24h_change || 0,
    }
    result["wemix-token"] = {
      brl: data["wemix-token"].brl,
      brl_24h_change: data["wemix-token"].brl_24h_change || 0,
    }

    return result
  } catch (error) {
    console.warn("API falhou, usando dados simulados:", error)
    // Retornar dados simulados com variação realística
...
const generateSimulatedData = () => {
  const bitcoinVariation = (Math.random() - 0.5) * 0.05 // ±2.5%
  const wemixVariation = (Math.random() - 0.5) * 0.1 // ±5%

  return {
    bitcoin: {
      brl: 580000 * (1 + bitcoinVariation),
      brl_24h_change: (Math.random() - 0.5) * 10, // ±5%
    },
    "wemix-token": {
      brl: 4.5 * (1 + wemixVariation),
      brl_24h_change: (Math.random() - 0.5) * 20, // ±10%
    },
  }
}
    ...
    async (showLoading = false) => {
      ...
      try {
        ...
        const formattedData: CryptoData[] = cryptos.map((crypto) => {
          const apiData = data[crypto.coingeckoId]
          if (!apiData) {
            // Se não há dados da API, usar dados simulados
            const simulatedPrice = crypto.basePrice * (1 + (Math.random() - 0.5) * 0.1)
            return {
              id: crypto.id,
              name: crypto.name,
              ...
              totalValue: simulatedPrice * crypto.amount,
              lastUpdated: now,
            }
          }
          return {
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <footer className="relative z-10 mt-16 py-8 text-center text-gray-500 border-t border-gray-800/50">
        <div className="font-mono">
          ...
          <p className="text-xs mt-2">Dados com variação realística</p>
        </div>
      </footer>
    </div>
  )
